...
@interface DashMainViewController () <UITableViewDelegate, UITableViewDataSource, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UITextFieldDelegate, GeneralPermissionsPopupProtocol, CongratulationsPopupProtocol, CMTabbarViewDelegate, CMTabbarViewDatasouce, SFSafariViewControllerDelegate> {
...
//    }
    ...
    [self.mainDashCoinsBtn addTarget:self action:@selector(openRewardsAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.showLeaderBtn addTarget:self action:@selector(openLeaderboardAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    //[self.chatButton addSubview:self.badgeView];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSArray *dd = defaults_object(@"detailsAllDrivingScores");
    if (dd.count != 0) {
...
- (void)startFetchStatisticData {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if ([defaults_object(@"onDemandTracking") boolValue]) {
            [self getRiskScoreForEachTag];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self getStatisticsDataForEachOnDemandTag];
            });
        }
    });
}
...
- (void)loadUserViewsDashboardMain {
    if ([defaults_object(@"needTrackingOn") boolValue]) {
        float reqDst = [defaults_object(@"statDistanceForScoring") floatValue];// detailsAllDrivingScores xself.appModel.statDistanceForScoring.floatValue;
        float userDst = [defaults_object(@"statSummaryDistance") floatValue]; //self.appModel.statSummaryDistance.floatValue;
        if ([defaults_object(@"needDistanceInMiles") boolValue]) {
...
- (void)getDashboardStatisticsIndividualAllTime:(NSDate *)startDate endDate:(NSDate *)endDate {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"statDistanceForScoring", @10);

            //self.appModel.statDistanceForScoring = @10;

            if ([defaults_object(@"needDistanceInMiles") boolValue]) {
                float miles = convertKmToMiles([defaults_object(@"needDistanceInMiles") floatValue]);
                defaults_set_object(@"statDistanceForScoring", @(miles));
            }

            defaults_set_object(@"statTrackCount", self.dashboard.TripsCount);
            defaults_set_object(@"statSummaryDistance", self.dashboard.MileageKm);
            defaults_set_object(@"statSummaryDuration", self.dashboard.DrivingTime);
...
- (void)getDashboardScoringsIndividualOnCurrentDay:(NSDate *)startDate endDate:(NSDate *)endDate {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"detailsScoreOverall", self.dashboard.SafetyScore);
            defaults_set_object(@"detailsScoreAcceleration", self.dashboard.AccelerationScore);
            defaults_set_object(@"detailsScoreDeceleration", self.dashboard.BrakingScore);
            defaults_set_object(@"detailsScoreSpeeding", self.dashboard.PhoneUsageScore);
            defaults_set_object(@"detailsScoreSpeeding", self.dashboard.SpeedingScore);
            defaults_set_object(@"detailsScoreTurn", self.dashboard.CorneringScore);
...
//            self.appModel.detailsScoreTurn = self.dashboard.CorneringScore;
            ...
            defaults_set_object(@"detailsScoreOverall", @0);
            defaults_set_object(@"detailsScoreAcceleration", @0);
            defaults_set_object(@"detailsScoreDeceleration", @0);
            defaults_set_object(@"detailsScoreSpeeding", @0);
            defaults_set_object(@"detailsScoreSpeeding", @0);
            defaults_set_object(@"detailsScoreTurn", @0);
...
- (void)getDashboardScoringsIndividual14daysDaily:(NSDate *)startDate14 endDate:(NSDate *)endDate14 {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"detailsAllDrivingScores", self.drivingDetails.Result);
...
- (void)getDashboardEcoDataAllTime {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"statEcoScoringFuel", self.ecoIndividual.EcoScoreFuel);
            defaults_set_object(@"statEcoScoringTyres", self.ecoIndividual.EcoScoreTyres);
            defaults_set_object(@"statEcoScoringBrakes", self.ecoIndividual.EcoScoreBrakes);
            defaults_set_object(@"statEcoScoringDepreciation", self.ecoIndividual.EcoScoreDepreciation);
            defaults_set_object(@"statEco", self.ecoIndividual.EcoScore);
            defaults_set_object(@"statPreviousEcoScoring", self.ecoIndividual.EcoScore);
...
//            self.appModel.statPreviousEcoScoring = self.ecoIndividual.EcoScore;
            ...
            //DEMO ECO
            defaults_set_object(@"statEcoScoringFuel", @90);
            defaults_set_object(@"statEcoScoringTyres", @90);
            defaults_set_object(@"statEcoScoringBrakes", @90);
            defaults_set_object(@"statEcoScoringDepreciation", @90);
            defaults_set_object(@"statEco", @90);
            defaults_set_object(@"statPreviousEcoScoring", @90);
...
- (void)getDashboardEcoDataAverageWeek {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"statWeeklyMaxSpeed", self.eco.MaxSpeedKmh);
            defaults_set_object(@"statWeeklyAverageSpeed", self.eco.AverageSpeedKmh);
            defaults_set_object(@"statWeeklyTotalKm", self.eco.MileageKm);
...
- (void)getDashboardEcoDataAverageMonth {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"statMonthlyMaxSpeed", self.eco.MaxSpeedKmh);
            defaults_set_object(@"statMonthlyAverageSpeed", self.eco.AverageSpeedKmh);
            defaults_set_object(@"statMonthlyTotalKm", self.eco.MileageKm);
...
- (void)getDashboardEcoDataAverageYear {
    ...
    [[LoginAuthApiRequest requestWithCompletion:^(id response, NSError *error) {
        ...
        if (!error && [response isSuccesful]) {
            ...
            defaults_set_object(@"statYearlyMaxSpeed", self.eco.MaxSpeedKmh);
            defaults_set_object(@"statYearlyAverageSpeed", self.eco.AverageSpeedKmh);
            defaults_set_object(@"statYearlyTotalKm", self.eco.MileageKm);
...
//        defaults_set_object(@"statDistanceForScoring", @(miles));
        ...
        float miles = convertKmToMiles([defaults_object(@"statSummaryDistance") floatValue]);
        self.progressBarDistance.progress = miles/[defaults_object(@"statDistanceForScoring") floatValue];
    } else {
        self.progressBarDistance.progress = [defaults_object(@"statSummaryDistance") floatValue]/[defaults_object(@"statDistanceForScoring") floatValue];
    }

    NSString *rounded = [NSString stringWithFormat:@"%.0f", [defaults_object(@"statSummaryDistance") floatValue]];
    ...
    self.needDistanceLabel.text = [NSString stringWithFormat:@"%@ %@ / %f %@", rounded, kmLocalize, [defaults_object(@"statDistanceForScoring") floatValue], kmLocalize];

    float tripsCount = [defaults_object(@"statTrackCount") floatValue];
    float mileageKm = [defaults_object(@"statSummaryDistance") floatValue];
    if ([defaults_object(@"needDistanceInMiles") boolValue]) {
        float miles = convertKmToMiles(mileageKm);
        mileageKm = miles;
    }

    float timeDriven = [defaults_object(@"statSummaryDistance") floatValue] / 60;
...
- (void)loadLinearChart:(NSInteger)type {
    ...
    NSArray *xs = defaults_object(@"detailsAllDrivingScores");

    NSMutableArray* chartData = [NSMutableArray arrayWithCapacity:xs.count];
    if (xs.count == 0) {
        ...
        for (int i=0; i < xs.count; i++) {
            ...
            DrivingDetailsObject *ddObj = xs[i];
            NSNumber *value;
            int count = +1;
            if (type == 0 && count == 1) {
        ...
        if (xs.count > 2)  {
            NSArray *ns = defaults_object(@"detailsAllDrivingScores");
            
            DrivingDetailsObject *trendObjLast = ns.lastObject;
            DrivingDetailsObject *trendObjPrevious = ns[ns.count - 2];
    ...
    if (xs.count == 0) {
        ...
        daysWeek = [NSMutableArray arrayWithCapacity:xs.count];
        for (int i=0; i < xs.count; i++) {
    ...
    if (xs.count <=4 && xs != nil) {
        _chartWithDates.horizontalGridStep = (int)xs.count;
    } else if (xs!= nil) {
        _chartWithDates.horizontalGridStep = (int)xs.count;
    } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.collectionViewCurve) {
        ...
        if (numSection == 0) {
            NSNumber *mm = defaults_object(@"detailsScoreOverall");
            [cell loadGauge:@99 curveName:localizeString(@"dash_overall")];

            if ([defaults_object(@"detailsScoreOverall") intValue] == 0) {
            ...
            NSNumber *mm = defaults_object(@"detailsScoreAcceleration");
            [cell loadGauge:@99 curveName:localizeString(@"dash_acceleration")];

            if ([defaults_object(@"detailsScoreAcceleration") intValue] == 0) {
                cell.trendArrowImg.hidden = YES;
            } else {
            ...
            NSNumber *mm = defaults_object(@"detailsScoreDeceleration");
            [cell loadGauge:@99 curveName:localizeString(@"dash_braking")];

            if ([defaults_object(@"detailsScoreDeceleration") intValue] == 0) {
                cell.trendArrowImg.hidden = YES;
            } else {
            ...
            NSNumber *mm = defaults_object(@"detailsScorePhoneUsage");
            [cell loadGauge:@99 curveName:localizeString(@"dash_phone")];

            if ([defaults_object(@"detailsScorePhoneUsage") intValue] == 0) {
                cell.trendArrowImg.hidden = YES;
            } else {
            ...
            NSNumber *mm = defaults_object(@"detailsScoreSpeeding");
            [cell loadGauge:@99 curveName:localizeString(@"dash_speeding")];

            if ([defaults_object(@"detailsScoreSpeeding") intValue] == 0) {
                cell.trendArrowImg.hidden = YES;
            } else {
            ...
            NSNumber *mm = defaults_object(@"detailsScoreTurn");
            [cell loadGauge:@99 curveName:localizeString(@"dash_cornering")];

            if ([defaults_object(@"detailsScoreTurn") intValue] == 0) {
                cell.trendArrowImg.hidden = YES;
            } else {
        ...
        if (indexPath.row == 0) {
            if ([defaults_object(@"needDistanceInMiles") boolValue]) {
                float mi1 = convertKmToMiles([defaults_object(@"statWeeklyAverageSpeed") floatValue]);
                float mi2 = convertKmToMiles([defaults_object(@"statWeeklyMaxSpeed") floatValue]);
                float mi3 = convertKmToMiles([defaults_object(@"statWeeklyTotalKm") floatValue]);
                ...
                cell.averageSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statWeeklyAverageSpeed")];
                cell.maxSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statWeeklyMaxSpeed")];
                float convertToAverage = (float) [defaults_object(@"statWeeklyTotalKm") floatValue]/self.statWeeklyTotalTrips.floatValue;
                if (isnan(convertToAverage))
            ...
            if ([defaults_object(@"needDistanceInMiles") boolValue]) {
                float mi1 = convertKmToMiles([defaults_object(@"statMonthlyAverageSpeed") floatValue]);
                float mi2 = convertKmToMiles([defaults_object(@"statMonthlyMaxSpeed") floatValue]);
                float mi3 = convertKmToMiles([defaults_object(@"statMonthlyTotalKm") floatValue]);
                cell.averageSpeed.text = [NSString stringWithFormat:@"%.0f mi/h", mi1];
                cell.maxSpeed.text = [NSString stringWithFormat:@"%.0f mi/h", mi2];
                ...
                cell.averageSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statMonthlyAverageSpeed")];
                cell.maxSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statMonthlyMaxSpeed")];
                float convertToAverage = (float) [defaults_object(@"statMonthlyTotalKm") floatValue]/self.statMonthlyTotalTrips.floatValue;
                if (isnan(convertToAverage))
                    cell.averageTrip.text = [NSString stringWithFormat:@"0.0 km"];
                else
                    cell.averageTrip.text = [NSString stringWithFormat:@"%.1f km", convertToAverage];
            ...
            if ([defaults_object(@"needDistanceInMiles") boolValue]) {
                float mi1 = convertKmToMiles([defaults_object(@"statYearlyAverageSpeed") floatValue]);
                float mi2 = convertKmToMiles([defaults_object(@"statYearlyMaxSpeed") floatValue]);
                float mi3 = convertKmToMiles([defaults_object(@"statYearlyTotalKm") floatValue]);
                cell.averageSpeed.text = [NSString stringWithFormat:@"%.0f mi/h", mi1];
                cell.maxSpeed.text = [NSString stringWithFormat:@"%.0f mi/h", mi2];
                ...
                cell.averageSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statYearlyAverageSpeed")];
                cell.maxSpeed.text = [NSString stringWithFormat:@"%@ km/h", defaults_object(@"statYearlyMaxSpeed")];
                float convertToAverage = (float) [defaults_object(@"statYearlyTotalKm") floatValue]/self.statYearlyTotalTrips.floatValue;
                if (isnan(convertToAverage))
                    cell.averageTrip.text = [NSString stringWithFormat:@"0.0 km"];
                else
                    cell.averageTrip.text = [NSString stringWithFormat:@"%.1f km", convertToAverage];
...
//            [[RPCSettings returnInstance] setAppName:localizeString(@"ZenRoad")];
//
//            #if USE_ELM_API
...
//            //NSString *page1_LBL3 = @"Allow ZenRoad to access your location?\n\n";
//            //NSString *page1_LBL_ALL2 = [NSString stringWithFormat:@"We use location services for tracking your mileage. Be enabling “Always while using App” on location, the app will be able to automatically determine when you have a drive."];
//            NSString *page1_LBL3 = @"";
...
//            //NSString *page3_LBL_ALL3 = [NSString stringWithFormat:@"%@ Would Like to Send You Notifications", localizeString(@"ZenRoad")];
...
//            NSMutableArray *page4_buttonsList = [[NSMutableArray alloc] initWithObjects:nil]; //OK
//
//
//            //PAGE BLUETOOTH WIZARD TEXT 1
//            NSString *pageBLE_LBL1 = @"Attention Required!\n\n";
//            NSString *pageBLE_LBL2 = @"“OK“";
//            NSString *pageBLE_LBL_ALL1 = [NSString stringWithFormat:@"You will receive a popup shortly for %@ to have access to your Bluetooth® even when you are not using the app.\n\nFor the app to work properly,\nchoose %@", localizeString(@"ZenRoad"), pageBLE_LBL2];
//
//            NSString *pageBLE_totalMainLbl1 = [NSString stringWithFormat:@"%@%@", pageBLE_LBL1, pageBLE_LBL_ALL1];
//
//            NSMutableAttributedString *pageBLE_completeText1 = [[NSMutableAttributedString alloc] initWithString:pageBLE_totalMainLbl1];
//            NSRange pageBLE_LBL1_range = [pageBLE_totalMainLbl1 rangeOfString:pageBLE_LBL1];
//            NSRange pageBLE_LBL2_range = [pageBLE_totalMainLbl1 rangeOfString:pageBLE_LBL2];
//
//            [pageBLE_completeText1 setAttributes:@{
//                                        NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:16.f],
//                                        NSParagraphStyleAttributeName: paragraphStyle
//                                        } range:NSMakeRange(0, pageBLE_completeText1.length)];
//            [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18.0f weight:UIFontWeightSemibold] range:pageBLE_LBL1_range];
//            [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18.0f weight:UIFontWeightSemibold] range:pageBLE_LBL2_range];
//
//            //PAGE BLUETOOTH WIZARD TEXT 2
//            //NSString *pageBLE_LBL3 = @"Allow ZenRoad to access your location?\n\n";
//            //NSString *pageBLE_LBL_ALL2 = [NSString stringWithFormat:@"We use location services for tracking your mileage. Be enabling “Always while using App” on location, the app will be able to automatically determine when you have a drive."];
//            NSString *pageBLE_LBL3 = @"";
//            NSString *pageBLE_LBL_ALL2 = [NSString stringWithFormat:@""];
//
//            NSTextAttachment *attachmentBLE = [[NSTextAttachment alloc] init];
//            attachmentBLE.image = [UIImage imageNamed:@"wizble_zr"];
//
//            if (IS_IPHONE_5 || IS_IPHONE_4) {
//                [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:16.0f weight:UIFontWeightSemibold] range:pageBLE_LBL1_range];
//                [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:16.0f weight:UIFontWeightSemibold] range:pageBLE_LBL2_range];
//                attachmentBLE.bounds = CGRectMake(0, 0, attachmentBLE.image.size.width - 120, attachmentBLE.image.size.height - 135);
//            } else if (IS_IPHONE_8) {
//                [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0f weight:UIFontWeightSemibold] range:pageBLE_LBL1_range];
//                [pageBLE_completeText1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0f weight:UIFontWeightSemibold] range:pageBLE_LBL2_range];
//                attachmentBLE.bounds = CGRectMake(0, 0, attachmentBLE.image.size.width - 75, attachmentBLE.image.size.height - 90);
//            } else if (IS_IPHONE_XS || IS_IPHONE_13_PRO) {
//                attachmentBLE.bounds = CGRectMake(0, 0, attachmentBLE.image.size.width - 70, attachmentBLE.image.size.height - 80);
//            } else {
//                attachmentBLE.bounds = CGRectMake(0, 0, attachmentBLE.image.size.width - 40, attachmentBLE.image.size.height - 50);
//            }
//            NSAttributedString *attrStringWithImagePageBLE = [NSAttributedString attributedStringWithAttachment:attachmentBLE];
//
//            NSString *pageBLE_totalMainLbl2 = [NSString stringWithFormat:@"%@%@", pageBLE_LBL3, pageBLE_LBL_ALL2];
//
//            NSMutableAttributedString *pageBLE_completeText2 = [[NSMutableAttributedString alloc] initWithString:pageBLE_totalMainLbl2];
//            NSRange pageBLE_LBL3_range = [pageBLE_totalMainLbl2 rangeOfString:pageBLE_LBL3];
//            [pageBLE_completeText2 setAttributes:@{
//                                        NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:16.f],
//                                        NSForegroundColorAttributeName: [UIColor darkGrayColor],
//                                        NSParagraphStyleAttributeName: paragraphStyle
//                                        } range:NSMakeRange(0, pageBLE_completeText2.length)];
//            [pageBLE_completeText2 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18.0f weight:UIFontWeightSemibold] range:pageBLE_LBL3_range];
//            [pageBLE_completeText2 appendAttributedString:attrStringWithImagePageBLE];
//
//            //PAGE BLUETOOTH WIZARD BUTTONS
//            NSMutableArray *pageBLE_buttonsList = [[NSMutableArray alloc] initWithObjects:nil]; //OK
//
//
//            //FINALLY WIZARD
...
//
//            RPCPageTexts *blePage = [RPCPageTexts initWithPageText:pageBLE_completeText1 alertText:pageBLE_completeText2 buttonsText:pageBLE_buttonsList selected:0];
//            NSArray<RPCPageTexts *> *customPages = [[NSArray alloc] initWithObjects:whileInUsePage, motionPage, blePage, pushPage, alwaysPage, nil];
...
//                [FIRAnalytics setUserID:[UserService sharedService].virtualDeviceToken? @"" : @""];
//                [[FIRCrashlytics crashlytics] setUserID:[UserService sharedService].virtualDeviceToken? @"" : @""];
//                [[FIRCrashlytics crashlytics] setCustomValue:[UserService sharedService].virtualDeviceToken forKey:@"installed_device_token"];
//                [FIRAnalytics logEventWithName:@"installed_device_token" parameters:@{@"installed_device_token": [UserService sharedService].virtualDeviceToken ? @"" : @""}];
//
...
- (void)mainCheckPermissions {
    ...
    if ([defaults_object(@"needTrackingOn") boolValue]) {
        ...
        if (permissionPopup.disabledGPS || permissionPopup.disabledMotion || permissionPopup.disabledPush) {
            ...
            if ([defaults_object(@"userDoneWizard") boolValue]) {
                float reqDst = [defaults_object(@"statDistanceForScoring") floatValue];
                float userDst = [defaults_object(@"statSummaryDistance") floatValue];
                if ([defaults_object(@"needDistanceInMiles") boolValue]) {
                    userDst = convertKmToMiles(userDst);
                }
...
#pragma mark - ONDEMAND Job Main buttons
...
#pragma mark - ONDEMAND Job Table buttons
...
#pragma mark - ONDEMAND Job Risk & Statistics Tags
...
- (void)getStatisticsDataForEachOnDemandTag {
    NSMutableArray *compJobs = defaults_object(@"completedJobs");
    NSMutableArray *tempForCompJobs = [compJobs mutableCopy];
    for (int i = 0; i < compJobs.count; i++) {
...
#pragma mark - ONDEMAND Job Table view datasource
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.jobsOnDutyAcceptTableView) {
        ...
        DashChallengeCell *challengeCell = [tableView dequeueReusableCellWithIdentifier:@"DashChallengeCell"];
...
#pragma mark - ONDEMAND Job Pause & Errors
...
//            //[self loadLastTrackData:ttk]; //TEST
//
//            dispatch_async(dispatch_get_main_queue(), ^{
...
//    }
    ...
    NSString *resParams = [NSString stringWithFormat:@"?apiKey=%@&w=%@&h=%@&nocp=%@&ml=%@&mtxc=%@&lc=%@&mthm=%@&t=%@&ppi=%@&lw=%@&f=%@&mfc=%@&m=%@", @"t8IWzdVDNpvnJ2XJSD-3I14-Kabb1Hl_t6kzWYTVyMg", imgW, @"360", @"1", @"eng", @"20", @"54C751", @"1", @"7", @"100", @"7", @"0", @"000000", specialMarkersPointsArrForHERE];
...
- (void)setupEcoViews {
    ...
    if (defaults_object(@"statRating") >= defaults_object(@"statPreviousRating")) {
        self.arrowPercentImg.image = [UIImage imageNamed:@"arrow_up_green"];
    } else
        self.arrowPercentImg.image = [UIImage imageNamed:@"arrow_down_red"];
    self.percentLbl.text = [NSString stringWithFormat:@"%.0f", [defaults_object(@"statEco") floatValue]];

    if ([defaults_object(@"statEco") floatValue] > 80) {
        self.roundPercentImg.image = [UIImage imageNamed:@"round_green"];
    } else if ([defaults_object(@"statEco") floatValue] > 60) {
        self.roundPercentImg.image = [UIImage imageNamed:@"round_yellow"];
    } else if ([defaults_object(@"statEco") floatValue] > 40) {
...
- (void)incrementProgressFuel:(NSTimer *)timer {
    int rate = [defaults_object(@"statEcoScoringFuel") intValue];
...
- (void)incrementProgressTires:(NSTimer *)timer {
    int rate = [defaults_object(@"statEcoScoringTyres") intValue];
...
- (void)incrementProgressBrakes:(NSTimer *)timer {
    int rate = [defaults_object(@"statEcoScoringBrakes") intValue];
...
- (void)incrementProgressTravelCost:(NSTimer *)timer {
    int rate = [defaults_object(@"statEcoScoringDepreciation") intValue];
...
- (void)setupEcoCollectionsForViews {
    ...
    NSBundle *bundle = [NSBundle bundleForClass:[UserActivityCell class]];
    
    [self.collectionViewActivity setContentOffset:CGPointMake(self.collectionViewActivity.bounds.size.width * 0, 0)];
    [self.collectionViewActivity registerNib:[UINib nibWithNibName:NSStringFromClass([UserActivityCell class]) bundle:bundle] forCellWithReuseIdentifier:NSStringFromClass([UserActivityCell class])];
    ...
    [self.demo_collectionViewActivity registerNib:[UINib nibWithNibName:NSStringFromClass([UserActivityCell class]) bundle:bundle] forCellWithReuseIdentifier:NSStringFromClass([UserActivityCell class])];
    [self.demo_collectionViewActivity reloadData];
}

- (void)setupRoundViews {
    self.mainBackgroundView.image = [UIImage imageNamed:@"feed_background"];
    ...
    if ([defaults_object(@"onDemandTracking") boolValue]) {
        self.demoDashboardView.layer.shadowRadius = 0;
    } else if ([defaults_object(@"trackingDisabledByUser") boolValue]) {
        self.demoDashboardView.layer.shadowRadius = 0;
    } else {
...
- (void)updateMainConstraints {
    float reqDst = [defaults_object(@"statDistanceForScoring") floatValue];// self.appModel.statDistanceForScoring.floatValue;
    float userDst = [defaults_object(@"statSummaryDistance") floatValue];// self.appModel.statSummaryDistance.floatValue;
    if ([defaults_object(@"needDistanceInMiles") boolValue]) {
        userDst = convertKmToMiles(userDst);
    }

//    self.mainDashboardDisabledTrackingView.layer.cornerRadius = 20;
//    self.mainDashboardDisabledTrackingView.layer.masksToBounds = NO;
//    self.mainDashboardDisabledTrackingView.layer.shadowOffset = CGSizeMake(0, 0);
//    self.mainDashboardDisabledTrackingView.layer.borderColor = [Color officialWhiteColor].CGColor;
//    self.mainDashboardDisabledTrackingView.layer.borderWidth = 1.4f;
...
- (void)setupTabBarTitles {
    ...
    [tabBarItem1 setImage:[[UIImage imageNamed:@"network_unselected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    [tabBarItem1 setSelectedImage:[[UIImage imageNamed:@"network_selected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    [tabBarItem1 setTitle:localizeString(@"leaderboard_title")];

    UITabBarItem *tabBarItem2 = [self.tabBarController.tabBar.items objectAtIndex:2];
    [tabBarItem2 setImage:[[UIImage imageNamed:@"exp_unselected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    [tabBarItem2 setSelectedImage:[[UIImage imageNamed:@"exp_selected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    [tabBarItem2 setTitle:localizeString(@"experience_title")];
    ...
    [tabBarItem4 setImage:[[UIImage imageNamed:@"more_unselected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    [tabBarItem4 setSelectedImage:[[UIImage imageNamed:@"more_selected"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
...
- (IBAction)openRewardsAction:(id)sender {
...
- (IBAction)openLeaderboardAction:(id)sender {
    [HapticHelper generateFeedback:FeedbackTypeImpactMedium];
...
- (IBAction)chatOpenAction:(id)sender {
    //[self sendEmailOptional];
}

- (IBAction)openAppSystemSettings:(id)sender {
    NSURL *URL = [NSURL URLWithString:[NSString stringWithFormat:@"%@", @"https://docs.telematicssdk.com/docs/permissions-in-ios#the-first-launch-ios-13-and-higher"]];
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"showTrackingModeFromSettingsRelease" object:self];
//    });
}


#pragma mark - Leaderboard source

- (void)fetchLeaderboard {
    //[self getLeaderboardUserGlobal];
}

- (void)fetchLeaderboardDetails {
//    [self getLeaderboardScores1];
//    [self getLeaderboardScores2];
//    [self getLeaderboardScores3];
//    [self getLeaderboardScores4];
//    [self getLeaderboardScores5];
//    [self getLeaderboardScores6];
//    [self getLeaderboardScores7];
//    [self getLeaderboardScores8];
//    [self getLeaderboardScores9];
}
...
#pragma mark - User Birthday implementation

- (void)requestRatingByUser {
    if (![defaults_object(@"needShowRatingView") boolValue]) {

        NSDate *date = [NSDate new];
        NSCalendar *calendar = [NSCalendar currentCalendar];
        NSObject *object = defaults_object(@"ratingDay");

        if (object != nil) {
            NSDate *savedAdDay = defaults_object(@"ratingDay");
            int differenceInDays = (int)([calendar ordinalityOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitEra forDate:date] - [calendar ordinalityOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitEra forDate:savedAdDay]);

            if (differenceInDays >= 1) {
                defaults_set_object(@"ratingDay", date);
                defaults_set_object(@"needShowRatingView", @(YES));
