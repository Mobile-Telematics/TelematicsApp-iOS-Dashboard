...
typedef void (^RequestResultBlock)(NSData *data, LAJSONModelError *error);
...
NSString* const kContentTypeLAJSON         = @"application/json";
...
@implementation LAJSONHTTPClient
...
+(NSString*)contentTypeForRequestString:(NSString*)requestString
{
    ...
    if (requestString.length>0 && [contentType isEqualToString:kContentTypeAutomatic]) {
        //check for "eventual" LAJSON array or dictionary
        ...
        if ([firstAndLastChar isEqualToString:@"{}"] || [firstAndLastChar isEqualToString:@"[]"]) {
            //guessing for a LAJSON request
            contentType = kContentTypeLAJSON;
        } else {
...
+(void)requestDataFromURL:(NSURL*)url method:(NSString*)method requestBody:(NSData*)bodyData headers:(NSDictionary*)headers handler:(RequestResultBlock)handler
{
    ...
    void (^completionHandler)(NSData *, NSURLResponse *, NSError *) = ^(NSData *data, NSURLResponse *origResponse, NSError *origError) {
        ...
        //convert an NSError to a LAJSONModelError
        if (origError) {
            error = [LAJSONModelError errorWithDomain:origError.domain code:origError.code userInfo:origError.userInfo];
        }
        ...
        //if not OK status set the err to a LAJSONModelError instance
        if (!error && (response.statusCode >= 300 || response.statusCode < 200)) {
            error = [LAJSONModelError errorBadResponse];
        }

        //if there was an error, assign the response to the LAJSONModel instance
        if (error) {
...
+(void)LAJSONFromURLWithString:(NSString*)urlString method:(NSString*)method params:(NSDictionary*)params orBodyString:(NSString*)bodyString completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString
                         method:method
                         params:params
...
+(void)LAJSONFromURLWithString:(NSString *)urlString method:(NSString *)method params:(NSDictionary *)params orBodyString:(NSString *)bodyString headers:(NSDictionary *)headers completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString
                         method:method
                         params:params
...
+(void)LAJSONFromURLWithString:(NSString*)urlString method:(NSString*)method params:(NSDictionary *)params orBodyData:(NSData*)bodyData headers:(NSDictionary*)headers completion:(LAJSONObjectBlock)completeBlock
{
    RequestResultBlock handler = ^(NSData *responseData, LAJSONModelError *error) {
        id jsonObject = nil;
        ...
        if (error==nil) {
            ...
            // So only create the LAJSON object if there is some response data.
        ...
        //step 4.5: cover an edge case in which meaningful content is return along an error HTTP status code
            ...
            //try to get the LAJSON object, while preserving the original error object
...
+(void)getLAJSONFromURLWithString:(NSString*)urlString completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString method:kHTTPMethodGET
                         params:nil
                   orBodyString:nil completion:^(id json, LAJSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}

+(void)getLAJSONFromURLWithString:(NSString*)urlString params:(NSDictionary*)params completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString method:kHTTPMethodGET
                         params:params
                   orBodyString:nil completion:^(id json, LAJSONModelError* e) {
...
+(void)postLAJSONFromURLWithString:(NSString*)urlString params:(NSDictionary*)params completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString method:kHTTPMethodPOST
                         params:params
                   orBodyString:nil completion:^(id json, LAJSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
...
+(void)postLAJSONFromURLWithString:(NSString*)urlString bodyString:(NSString*)bodyString completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString method:kHTTPMethodPOST
                         params:nil
                   orBodyString:bodyString completion:^(id json, LAJSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}

+(void)postLAJSONFromURLWithString:(NSString*)urlString bodyData:(NSData*)bodyData completion:(LAJSONObjectBlock)completeBlock
{
    [self LAJSONFromURLWithString:urlString method:kHTTPMethodPOST
                         params:nil
                   orBodyString:[[NSString alloc] initWithData:bodyData encoding:defaultTextEncoding]
                                 completion:^(id json, LAJSONModelError* e) {
                       if (completeBlock) completeBlock(json, e);
                   }];
}
