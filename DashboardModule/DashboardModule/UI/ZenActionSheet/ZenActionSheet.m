...
//
//  Created by pp@datamotion.ai on 24.02.21.
//  Copyright © 2022 DATA MOTION PTE. LTD. All rights reserved.
//

#import "ZenActionSheet.h"

typedef NS_ENUM(NSInteger, ZenActionSheetArrowDirection) {
    ZenActionSheetArrowDirectionNone,
    ZenActionSheetArrowDirectionDown,
    ZenActionSheetArrowDirectionLeft,
    ZenActionSheetArrowDirectionUp,
    ZenActionSheetArrowDirectionRight
};

const CGFloat kZenActionSheetButtonHeight  = 60.0f;
const CGFloat kZenActionSheetDefaultWidth  = 300.0f;
const CGFloat kZenActionSheetBorderRadius  = 7.0f;
const CGFloat kZenActionSheetTitlePadding  = 10.0f;
const CGFloat kZenActionSheetCompactMargin = 20.0f;
const CGFloat kZenActionSheetArrowBase     = 36.0f;
const CGFloat kZenActionSheetArrowHeight   = 13.0f;
const CGFloat kZenActionSheetScreenPadding = 20.0f;
...
@property (nonatomic, assign) ZenActionSheetArrowDirection arrowDirection;
...
- (instancetype)initWithStyle:(ZenActionSheetStyle)style
{
    if (self = [super init]) {
        _style = style;
        [self setUp];
...
- (void)setColorsForStyle:(ZenActionSheetStyle)style {
    if (style == ZenActionSheetStyleDark) {
...
- (void)layoutSubviews
{
    ...
    if (self.compactLayout) {
        ...
        frame.origin.y = (self.frame.size.height - (kZenActionSheetCompactMargin + contentInset)) - frame.size.height;
        self.cancelButton.frame = frame;
        
        frame = self.containerView.frame;
        frame.size.width = self.width;
        frame.origin.x = (self.frame.size.width - frame.size.width) * 0.5f;
        frame.origin.y = (self.cancelButton.frame.origin.y - kZenActionSheetCompactMargin) - frame.size.height;
        ...
        ZenActionSheetArrowDirection direction = [self bestArrowDirectionForPresentationRect:presentationRect];
...
{
    CGRect rect = CGRectMake(0, 0, (kZenActionSheetBorderRadius * 2) + 1, (kZenActionSheetBorderRadius * 2) + 1);
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:rect
                                               byRoundingCorners:corners
                                                     cornerRadii:(CGSize){kZenActionSheetBorderRadius, kZenActionSheetBorderRadius}];
    ...
    UIEdgeInsets edgeInsets = (UIEdgeInsets){kZenActionSheetBorderRadius, kZenActionSheetBorderRadius, kZenActionSheetBorderRadius, kZenActionSheetBorderRadius};
...
- (UIImage *)arrowImageForDirection:(ZenActionSheetArrowDirection)direction color:(UIColor *)color
{
    CGSize size = CGSizeZero;
    if (direction == ZenActionSheetArrowDirectionDown || direction == ZenActionSheetArrowDirectionUp) {
        size = (CGSize){kZenActionSheetArrowBase, kZenActionSheetArrowHeight};
    }
    else {
        size = (CGSize){kZenActionSheetArrowHeight, kZenActionSheetArrowBase};
    }
    ...
    UIGraphicsBeginImageContextWithOptions(size, NO, 0.0f);
    {
        ...
        switch (direction) {
            case ZenActionSheetArrowDirectionDown:
                ...
                break;
            case ZenActionSheetArrowDirectionUp:
                ...
                break;
            case ZenActionSheetArrowDirectionRight:
                CGPathMoveToPoint(arrowPath, NULL, CGRectGetMinX(arrowRect), CGRectGetMinY(arrowRect));
                CGPathAddCurveToPoint(arrowPath, NULL,
                ...
                break;
            case ZenActionSheetArrowDirectionLeft:
            default:
...
{
    CGRect buttonFrame = (CGRect){0,0,self.width,kZenActionSheetButtonHeight};
...
- (void)setUpCancelButton
{
    ...
    self.cancelButton.frame = (CGRect){0,0,self.width,kZenActionSheetButtonHeight};
...
- (void)setUpDestructiveButton
{
    ...
    self.destructiveButton.frame = (CGRect){0,0,self.width,kZenActionSheetButtonHeight};
...
- (void)setUpHeaderView
{
    ...
    if (self.headerView) {
        ...
        frame.size = [titleLabel sizeThatFits:(CGSize){self.width-(kZenActionSheetTitlePadding*2.0f), NSIntegerMax}];
        frame.origin = (CGPoint){kZenActionSheetTitlePadding, kZenActionSheetTitlePadding};
        
        CGRect backgroundFrame = self.headerBackgroundView.frame;
        backgroundFrame.size.width = self.width;
        backgroundFrame.size.height = frame.size.height + (kZenActionSheetTitlePadding * 2.0f);
...
- (void)setUpContainerWidth
{
    if (self.compactLayout == NO) {
        self.width = kZenActionSheetDefaultWidth;
        return;
    }
...
- (void)setUpContainer
{
    ...
    frame.size.height += (self.buttonTitles.count * kZenActionSheetButtonHeight);
    ...
    if (self.headerBackgroundView) {
        ...
        height = kZenActionSheetButtonHeight;
    }
...
- (ZenActionSheetArrowDirection)bestArrowDirectionForPresentationRect:(CGRect)presentationRect
{
    CGFloat height = CGRectGetHeight(self.containerView.frame) + kZenActionSheetArrowHeight;
    CGFloat width = kZenActionSheetDefaultWidth + kZenActionSheetArrowHeight;
    
    if (CGRectGetMinY(presentationRect) - height > 0) {
        return ZenActionSheetArrowDirectionDown;
    }
    else if (CGRectGetMinX(presentationRect) - width > 0) {
        return ZenActionSheetArrowDirectionRight;
    }
    else if (CGRectGetMaxX(presentationRect) + width <= CGRectGetWidth(self.frame)) {
        return ZenActionSheetArrowDirectionLeft;
    }
    else if (CGRectGetMaxY(presentationRect) + width <= CGRectGetHeight(self.frame)) {
        return ZenActionSheetArrowDirectionUp;
    }

    return ZenActionSheetArrowDirectionNone;
}

- (CGRect)frameOfContainerViewWithArrowDirection:(ZenActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect
{
    ...
    switch (direction) {
        case ZenActionSheetArrowDirectionDown:
            frame.origin.y = CGRectGetMinY(presentationRect) - (CGRectGetHeight(frame) + kZenActionSheetArrowHeight);
        ...
        case ZenActionSheetArrowDirectionRight:
            ...
            frame.origin.x = CGRectGetMinX(presentationRect) - (CGRectGetWidth(frame) + kZenActionSheetArrowHeight);
            break;
        case ZenActionSheetArrowDirectionUp:
            frame.origin.y = CGRectGetMaxY(presentationRect) + kZenActionSheetArrowHeight;
            frame.origin.x = CGRectGetMidX(presentationRect) - (CGRectGetWidth(frame) * 0.5f);
            break;
        case ZenActionSheetArrowDirectionLeft:
            frame.origin.y = CGRectGetMidY(presentationRect) - (CGRectGetHeight(frame) * 0.5f);
            frame.origin.x = CGRectGetMaxX(presentationRect) + kZenActionSheetArrowHeight;
            break;
        default:
            break;
    }
    
    if (frame.origin.x < kZenActionSheetScreenPadding) {
        frame.origin.x = kZenActionSheetScreenPadding;
    }
    
    if (frame.origin.y < kZenActionSheetScreenPadding) {
        frame.origin.y = kZenActionSheetScreenPadding;
    }
    
    if (CGRectGetMaxX(frame) > (boundSize.width - kZenActionSheetScreenPadding)) {
        frame.origin.x = (boundSize.width - kZenActionSheetScreenPadding) - CGRectGetWidth(frame);
    }
        
    if (CGRectGetMaxY(frame) > (boundSize.height - kZenActionSheetScreenPadding)) {
        frame.origin.y = (boundSize.height - kZenActionSheetScreenPadding) - CGRectGetHeight(frame);
    }
        
    return frame;
}

- (CGRect)frameOfArrowViewWithDirection:(ZenActionSheetArrowDirection)direction presentationRect:(CGRect)presentationRect
{
    ...
    switch (direction) {
        ...
        case ZenActionSheetArrowDirectionRight:
            ...
            break;
        case ZenActionSheetArrowDirectionUp:
            frame.origin.y = -kZenActionSheetArrowHeight;
        ...
        case ZenActionSheetArrowDirectionLeft:
            offset = CGRectGetMidY(presentationRect) - CGRectGetMidY(containerFrame);
            frame.origin.y = (CGRectGetMidY(containerBounds) - (CGRectGetHeight(frame) * 0.5f)) + offset;
            frame.origin.x = -kZenActionSheetArrowHeight;
            break;
        default:
            break;
    }
    ...
    if (direction == ZenActionSheetArrowDirectionDown || direction == ZenActionSheetArrowDirectionUp) {
        frame.origin.x = MAX(kZenActionSheetBorderRadius, frame.origin.x);
        CGFloat minX = (CGRectGetWidth(containerBounds) - kZenActionSheetBorderRadius) - kZenActionSheetArrowBase;
        frame.origin.x = MIN(minX, frame.origin.x);
    }
    else if (direction == ZenActionSheetArrowDirectionLeft || direction == ZenActionSheetArrowDirectionRight) {
        frame.origin.y = MAX(kZenActionSheetBorderRadius, frame.origin.y);
        CGFloat minY = (CGRectGetHeight(containerBounds) - kZenActionSheetBorderRadius) - kZenActionSheetArrowBase;
...
- (UIColor *)bestColorForArrowWithDirection:(ZenActionSheetArrowDirection)direction
{
    switch (direction) {
        case ZenActionSheetArrowDirectionDown:
            if (self.destructiveTitle) {
...
- (void)setStyle:(ZenActionSheetStyle)style
{
    if (style == _style) {
        return;
    }
